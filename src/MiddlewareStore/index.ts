...
// this is updated version of adonis core class
// we have updated the resolveMiddleware to allow functions
// and changed middleware invoked methods from handle to wsHandle
export class MiddlewareStore implements WsMiddlewareStoreContract {
  /**
  ...
  private resolveMiddleware(middleware: MiddlewareHandler): ResolvedMiddlewareHandler {
    if (typeof middleware === 'function') {
      return middleware.length > 0
        ? {
            type: 'function',
            value: middleware as FunctionMiddlewareHandler,
            args: [],
          }
        : {
            type: 'lazy-import',
            value: middleware as LazyImportMiddlewareHandler,
            args: [],
          }
    }

    return Object.assign(this.resolver.resolve(`${middleware}.wsHandle`), { args: [] })
  }
  ...
  public async invokeMiddleware(
    ...
    const args: any[] = [params[0], params[1], middleware.args]
    ...
    if (middleware.type === 'lazy-import') {
      ...
      return this.container.callAsync(middlewareInstance, 'wsHandle', args)
    }
